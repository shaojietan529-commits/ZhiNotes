...
function buildHealthAreas(): ModuleHealthArea[] {
  return [
    ...
    },
    {
      ...
      module_ids: ["sync"],
      evidence:
        "Sync module contains local backup export, sync queue visibility, payload preview, replay plan, account/session boundary, cloud schema plan, deployment target, smoke test plan, and permission contracts.",
      next_action:
        "Resolve GitHub credentials, configure disposable Supabase/Vercel preview, prove rollback, implement conflict UI, and keep writes disabled until owner confirmation.",
      privacy_boundary:
