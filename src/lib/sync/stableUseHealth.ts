...
const STABLE_USE_TWO_DAY_SYNC_POLICY: StableUseTwoDaySyncPolicy = {
  ...
  uncleared_required_receipt_blocks_device_handoff: true,
  user_facing_copy:
    "48 小时目标是私有 beta 可稳定使用：写作先本地保存，页面/每日纪要/ZhiHui/数据库/文件元数据/设置的同步状态必须可见；页面/数据库必需回执必须 ok 且 pendingAfter=0，失败回执或 pendingAfter 未清不能换设备；只有真实两设备 smoke 跑通并且 pending、failed、manual review 清零后，才能声称全平台同步可用。",
};
