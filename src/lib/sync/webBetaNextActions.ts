...
function buildActions(
  ...
  return [
    ...
    fromGate({
      ...
      fallbackEvidence:
        "Local role decisions, a metadata-only permission check envelope, local validator fixtures, a server permission test matrix, and a dedicated disabled /api/permissions/check schema guard exist, but server endpoints do not enforce roles.",
      fallbackAction:
        "Move Owner, Researcher, and Viewer decisions into authenticated server checks only after the route runs validator-backed forbidden payload rejection, server matrix tests, and high-risk actions stay owner-confirmed.",
      status: "ready-to-build",
