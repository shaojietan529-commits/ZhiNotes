  ...
  const gates: WebBetaReadinessGate[] = [
    {
      ...
      category: "local",
      evidence:
        "Backup JSON, workspace ZIP, Markdown export, sync queue export, sync replay test plan export, disposable replay confirmation receipt export, empty-fixture replay package export, disposable replay harness preflight export, restore rollback plan export, restore write-back contract export, permission policy export, permission decision export, account session boundary export, environment preflight export, cloud schema migration plan export, cloud migration SQL draft export, launch checklist export, contract export, and readiness export are available locally.",
      nextAction:
    ...
    },
    {
      ...
      evidence: input.syncPayloadPreview
        ? `A local sync payload preview is available for ${input.syncPayloadPreview.summary.pending_count} pending rows; it is metadata-only and requires confirmation. Disposable replay also has a local confirmation receipt, empty-fixture package, and harness preflight before any empty-data replay can run.`
      ...
      nextAction:
        "Make payload preview, disposable replay receipt, empty-fixture package, and harness preflight required confirmation screens before enabling cloud push or empty-data replay.",
    },
    {
    ...
    },
    {
      ...
      evidence: input.conflictReview
        ? `A local conflict review scaffold covers ${input.conflictReview.summary.surfaces} surfaces, and sync UI now keeps side-by-side review plus remote baseline request, staging, schema/cursor proof, disposable replay/RLS proof planning, disposable replay confirmation receipt, empty-fixture replay package, and harness preflight local-only. It still does not read remote data or merge changes.`
        : "Conflict policies exist for pages, database rows, files, comments, permissions, and restore, but no review UI is implemented.",
      nextAction:
        "Export owner confirmation, empty-fixture package, and harness preflight, then run disposable replay/RLS proof on empty workspace fixtures, prove rollback, and require owner confirmation before supporting multi-device editing.",
    },
    {
