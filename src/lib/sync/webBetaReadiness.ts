  ...
  const gates: WebBetaReadinessGate[] = [
    ...
    },
    {
      ...
      evidence: input.conflictReview
        ? `A local conflict review scaffold covers ${input.conflictReview.summary.surfaces} surfaces, but it does not read remote data or merge changes.`
        : "Conflict policies exist for pages, database rows, files, comments, permissions, and restore, but no review UI is implemented.",
      nextAction:
        "Add remote baseline detection and side-by-side resolution screens before supporting multi-device editing.",
    },
    {
