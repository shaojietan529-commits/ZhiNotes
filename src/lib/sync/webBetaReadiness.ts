  ...
  const gates: WebBetaReadinessGate[] = [
    {
      ...
      category: "local",
      evidence:
        "Backup JSON, workspace ZIP, Markdown export, sync queue export, sync replay test plan export, disposable replay confirmation receipt export, empty-fixture replay package export, restore rollback plan export, restore write-back contract export, permission policy export, permission decision export, account session boundary export, environment preflight export, cloud schema migration plan export, cloud migration SQL draft export, launch checklist export, contract export, and readiness export are available locally.",
      nextAction:
    ...
    },
    {
      ...
      evidence: input.syncPayloadPreview
        ? `A local sync payload preview is available for ${input.syncPayloadPreview.summary.pending_count} pending rows; it is metadata-only and requires confirmation. Disposable replay also has a local confirmation receipt and empty-fixture package before any empty-data replay can run.`
      ...
      nextAction:
        "Make payload preview, disposable replay receipt, and empty-fixture package required confirmation screens before enabling cloud push or empty-data replay.",
    },
    {
