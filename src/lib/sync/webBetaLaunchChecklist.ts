  ...
  return [
    ...
    },
    {
      ...
      evidence: input.permissionDecisionReport
        ? `${input.disabledApiStubs} Web Beta API routes are disabled stubs; local permission decisions cover ${input.permissionDecisionReport.summary.matrix_decisions} role/resource/action combinations, the sync UI exports a metadata-only permission check envelope, and /api/permissions/check returns a dedicated disabled schema guard, but server enforcement is disabled.`
        : `${input.disabledApiStubs} Web Beta API routes are disabled stubs; auth/session routes do not create sessions.`,
      required_action:
        "Choose auth provider, session storage, workspace membership model, permission envelope validation, forbidden-payload rejection tests, and server-side role enforcement.",
    },
    {
