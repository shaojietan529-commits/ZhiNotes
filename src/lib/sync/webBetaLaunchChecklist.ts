  ...
  return [
    ...
    },
    {
      ...
      evidence: input.conflictReview
        ? `Conflict scaffold covers ${input.conflictReview.summary.surfaces} surfaces, and sync UI includes local-only side-by-side, baseline request, staging, schema/cursor proof, disposable replay/RLS proof contracts, disposable replay confirmation receipt, empty-fixture replay package, and harness preflight. It still does not read remote baselines.`
        : "No local conflict review scaffold is available.",
      required_action:
        "Export owner confirmation, empty-fixture package, and harness preflight, run disposable replay/RLS proof on empty workspace fixtures, prove rollback, and require owner confirmation before multi-device editing.",
    },
    {
    ...
    },
    {
      ...
      required_action:
        "Make payload preview, replay receipts, empty-fixture packages, and harness preflights mandatory before cloud sync, disposable replay, AI execution, external asset loading, sharing, or bulk delete.",
    },
    {
