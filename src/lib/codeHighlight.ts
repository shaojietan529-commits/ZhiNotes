...
function getLanguageConfig(language: string): LanguageConfig {
  ...
  if (language === "latex" || language === "bibtex") {
    return {
      ...
      literals: new Set(),
      builtins: toSet(["alpha", "beta", "frac", "sum", "textbf", "textit"]),
      lineComments: ["%"],
      blockComments: [],
      propertyWords: true,
    };
  ...
  if (language === "mermaid") {
    return {
      ...
      literals: toSet(["false", "true"]),
      builtins: toSet(["td", "tb", "lr", "rl"]),
      lineComments: ["%%"],
      blockComments: [],
      propertyWords: true,
    };
  }
  if (language === "julia" || language === "sas" || language === "stata") {
    return {
      keywords: toSet([
        "begin",
        "by",
      ...
      literals: toSet(["false", "missing", "null", "true"]),
      builtins: toSet(["display", "import", "include", "mean", "print", "println", "sum"]),
      lineComments:
        language === "stata" ? ["*", "//"] : language === "sas" ? ["*"] : ["#"],
      blockComments: language === "stata" ? [] : [["/*", "*/"]],
      propertyWords: true,
    };
  }
  if (language === "ris") {
    return {
      keywords: toSet(["au", "do", "er", "jo", "kw", "n1", "py", "sp", "ti", "t1", "ty", "ur"]),
      literals: new Set(),
      builtins: new Set(),
      lineComments: [],
      blockComments: [],
