...
- Primary workspaces (sidebar top-level): 每日纪要 (`/daily`, month calendar over date-titled child pages with a Notion-style Date/要点/Summary template), 产业链研究 (`/industry-chain`, hierarchical page-tree board where every node is a real page, with inline double-click rename), 会议日程 (`/schedule`, month calendar over meeting pages carrying date/time/platform/organizer properties; manual add only — auto record/transcribe/publish is a gated placeholder for the Codex meeting agent). Each is backed by a singleton local root page via `src/lib/pages/moduleWorkspaces.ts`; all other modules live in a collapsible "备选模块" section. Verifier: `npm run verify:module-workspaces`.
...
- Account-scoped page cloud sync: `/api/pages/account-sync` (session-gated) stores one cloud copy of the page tree per account email (per-page KV records + manifest index, last-write-wins by `updated_at`, server rejects stale overwrites, tombstones propagate deletes). Client engine `src/lib/pages/accountPageSync.ts` is opt-in only: a toggle on `/account` (localStorage flag + confirm dialog) gates everything; `usePageCloudSync` (mounted in Sidebar, ☁️ chip on the 账号 row) reconciles on load + every 60s. Module workspace roots merge by title (smallest id wins) so two domains converge without duplicate 每日纪要/产业链研究/会议日程 roots. Synced fields: title/body HTML/hierarchy/position/icon/properties/cover (covers >300KB skipped); NOT synced: databases, files, comments, versions, content_yjs. Verifier: `npm run verify:account`.
