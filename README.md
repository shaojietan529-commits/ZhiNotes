...
- Use the notes 格式入口 panel to route Markdown notes, HTML visual reports,
  ...
  the notes review sequence and Cmd/Ctrl+K command palette.
...
- Use wide local table views with the row number and title columns frozen on the
  left, and optionally freeze up to three extra properties per table view. This
  keeps long investment trackers readable without changing field order or row
  values.
...
- Use the meeting intake desk to create one local meeting tracker row from a
  ...
  call AI.
- ZhiHui glossary responses are read-only and `Cache-Control: no-store`.
...
- ZhiHui agent queue consumers must use the lease-aware ACK contract. A runner
  ...
  return. Poll, claim, and enqueue responses mirror `operation`, `queueAction`,
  `queueReadStatus`, and `queueWriteStatus` at the top level; plain polling is
  `not_started`, claim mode reports `lease_claim_completed` or
  `not_needed_no_available_jobs`, and enqueue reports `completed` or
  `not_needed_existing_job` so clients can tell when a runner lease or page
...
- Validate future cloud manifest compare requests locally. `npm run
  ...
  values.
- Validate future cloud manifest compare responses locally. `npm run
  verify:cloud-manifest-response` evaluates the metadata-only response
  validator against synthetic fixtures, rejects page bodies, database values,
...
- `npm run verify:private-alpha` runs the fast P0 stable-use gate for small
  ...
  module registry contracts, module center stable-use status, database
  ...
  sync center, module hub, database, report, file, company research, meeting,
  project, research graph, knowledge base, industry chain, calendar, portfolio,
  and page entrypoints. It prints a local receipt, does
