...
function assertOrderedSnippets(sourceLabel, source, snippets, message) {
  let cursor = 0;
...
function run() {
  ...
  for (const [snippet, message] of [
    ...
    [
      ...
      "Daily notes must track mounted state for background load guards.",
    ],
    [
      "mountedRef.current = false",
      "Daily notes must mark the route unmounted during cleanup.",
    ],
    [
  ...
  for (const [snippet, message] of [
    ...
    [
      ...
      "Daily notes initial cloud metadata correction must run as a foreground-aware delayed idle task.",
    ],
    [
      "DAILY_INITIAL_CLOUD_RECHECK_DELAY_MS,\n      DAILY_INITIAL_CLOUD_RECHECK_IDLE_TIMEOUT_MS",
  ...
  for (const [snippet, message] of [
    ...
    [
      ...
      "Meeting schedule initial fallback must re-check whether the calendar is still empty before running heavier metadata fallback.",
    ],
    [
    ...
    [
      ...
      "Meeting schedule initial fallback metadata fill must run after a first-paint-aware recheck.",
    ],
    [
      "const cancelCloudRecheck = scheduleMeetingForegroundAwareIdleTask(",
      "Meeting schedule initial cloud metadata correction must run as a foreground-aware delayed idle task.",
    ],
    [
      "MEETING_INITIAL_CLOUD_RECHECK_DELAY_MS,\n      MEETING_INITIAL_CLOUD_RECHECK_IDLE_TIMEOUT_MS",
  ...
  assertIncludes(
    ...
    usePage,
    "const foregroundQuietUntilRef = useRef(0);",
    "Page opening must track a foreground quiet window so local editing is not interrupted by background reloads."
  );
  assertIncludes(
    files.usePage,
    usePage,
    "const foregroundPageIdRef = useRef<string | null>(pageId);",
    "Page opening foreground quiet windows must be scoped to the current page id so switching pages stays fast."
  );
  assertIncludes(
    files.usePage,
    usePage,
  ...
  assertIncludes(
    ...
    "Page opening revision refreshes must re-check the foreground quiet window when timers fire."
  );
  assertIncludes(
    files.usePage,
    usePage,
    "window.setTimeout(runWhenQuiet, foregroundDelay)",
    "Page opening cross-tab fallback refreshes must defer again if local editing starts after scheduling."
  );
  assertIncludes(
    files.usePage,
    usePage,
    "if (foregroundPageIdRef.current !== pageId) return 0;",
    "Page opening must not apply a previous page's foreground quiet window to a newly opened page."
  );
  assertIncludes(
    files.usePage,
    usePage,
  ...
  for (const [snippet, message] of [
    ...
    assertIncludes(files.accountServer, accountServer, snippet, message);
  }
  ...
  for (const [snippet, message] of [
    ...
    assertIncludes(files.cloudSupabaseRest, cloudSupabaseRest, snippet, message);
  }
  ...
  for (const [snippet, message] of [
    ...
    assertIncludes(files.cloudClientSession, cloudClientSession, snippet, message);
  }
  ...
  for (const [snippet, message] of [
    ...
    assertIncludes(files.authRefreshRoute, authRefreshRoute, snippet, message);
  }
  for (const [snippet, message] of [
    [
  ...
  for (const [snippet, message] of [
    ...
    [
      ...
      "Portfolio browser action timeouts must use a typed error.",
    ],
    [
    ...
    [
      "本地组合数据已保留，可稍后重试。",
      "Portfolio browser action timeout copy must reassure users that local portfolio data is preserved.",
    ],
  ]) {
    assertIncludes(
  ...
  for (const [snippet, message] of [
    ...
    assertIncludes(files.accountShell, accountShell, snippet, message);
  }
  if (
    !(
  ...
  for (const [file, source, snippet, message] of [
    ...
    assertIncludes(file, source, snippet, message);
  }
  assertExcludes(
    files.knowledgeAccountSyncRoute,
    knowledgeAccountSyncRoute,
  ...
  for (const [snippet, message] of [
    ...
    [
      ...
      "SyncShell cloud API timeouts must use a typed retryable error.",
    ],
    [
    ...
    [
      ...
      "SyncShell workspace actions must use the bounded cloud API helper.",
    ],
    [
  ...
  assertIncludes(
    files.sidebar,
    sidebar,
    "fetchAccountSession({ force: options.force })",
    "Sidebar account label refresh must bypass the short account-session cache after account profile changes."
  );
  assertIncludes(
    files.sidebar,
    sidebar,
  ...
  assertIncludes(
    ...
    "Sidebar account label should explain cross-tab profile refresh as cloud confirmation, not a logout."
  );
  assertIncludes(
    files.sidebar,
    sidebar,
  ...
  assertIncludes(
    ...
    "Sidebar account profile events must use a wrapper that passes forced refresh options."
  );
  assertIncludes(
    files.sidebar,
    sidebar,
