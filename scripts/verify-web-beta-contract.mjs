...
function run() {
  ...
  for (const [snippet, message] of [
    ...
    [
      "does not send network requests",
      "Meeting calendar status privacy boundary must explicitly exclude network requests.",
    ],
    [
      "does not write server data",
      "Meeting calendar status privacy boundary must explicitly exclude server writes.",
    ],
  ]) {
    assertSourceIncludes(
      files.meetingCalendarLoadStatus,
  ...
  for (const [snippet, message] of [
    ["content_text", "Meeting calendar status must not read page content text."],
    ["content_yjs", "Meeting calendar status must not read page Yjs content."],
    ["joinUrl", "Meeting calendar status must not read join URLs."],
    ["meetingId", "Meeting calendar status must not read meeting ids."],
    ["entry.passcode", "Meeting calendar status must not read passcodes."],
    ["field_values", "Meeting calendar status must not read database row values."],
    ["comment.body", "Meeting calendar status must not read comment bodies."],
    ["file.dataUrl", "Meeting calendar status must not read file bytes."],
    ["fetch(", "Meeting calendar status must not call network APIs."],
  ...
  for (const [snippet, message] of [
    ...
    [
      ...
      "usePage must identify explicit local-first navigation sources before scheduling body hydration.",
    ],
    [
      "getPageLocalBodyHydrationPriority",
      "usePage must derive a body hydration priority from the route handoff source.",
    ],
    [
      "localBodyHydrationPriority",
      "usePage must pass the navigation-aware local body hydration priority into the scheduler.",
    ],
    [
      "PAGE_INTERACTIVE_LOCAL_BODY_HYDRATION_DELAY_MS = 24",
      "usePage must keep explicit-click local body hydration nearly immediate without blocking first paint.",
    ],
    [
      "PAGE_INTERACTIVE_LOCAL_BODY_HYDRATION_IDLE_MS = 80",
      "usePage must keep explicit-click local body hydration bounded even when the browser is busy.",
    ],
    [
      'priority === "interactive"',
      "usePage must have a distinct interactive body-hydration path for clicked pages.",
    ],
    [
    ...
    [
      ...
      "usePage local body hydration must use the content-text projection instead of getPage SELECT *.",
    ],
    [
  ...
  assertSourceExcludes(
    ...
    "getPage(pageId)",
    "usePage local body hydration must not read content_yjs blobs through getPage SELECT *."
  );
  assertSourceExcludes(
    files.usePage,
    usePage,
  ...
  for (const [snippet, message] of [
    ...
    assertSourceIncludes(files.accountCloudSyncGate, accountCloudSyncGate, snippet, message);
  }
  for (const [snippet, message] of [
    [
  ...
  for (const [snippet, message] of [
    [
      ...
      "Daily calendar initial cloud correction must have an explicit delay so first paint remains local-first.",
    ],
    [
      "DAILY_INITIAL_CLOUD_RECHECK_IDLE_TIMEOUT_MS",
      "Daily calendar initial cloud correction must have a bounded idle timeout.",
    ],
    [
      "const DAILY_CLOUD_METADATA_RECHECK_DELAY_MS = 900",
      "Daily calendar follow-up cloud metadata correction must start quickly after local updates.",
    ],
    [
      "const DAILY_INITIAL_CLOUD_RECHECK_DELAY_MS = 120",
      "Daily calendar initial cloud correction must start shortly after local first paint.",
    ],
    [
      "const DAILY_INITIAL_CLOUD_RECHECK_IDLE_TIMEOUT_MS = 900",
      "Daily calendar initial cloud correction idle timeout must stay bounded for cloud-first users.",
    ],
    [
      "void load({\n        includeCloud: false,\n        interruptCloud: false,\n        preserveVisibleNotes: true,\n      });",
  ...
  for (const [snippet, message] of [
    [
      ...
      "Meeting calendar initial cloud correction must have an explicit delay so first paint remains local-first.",
    ],
    [
      "MEETING_INITIAL_CLOUD_RECHECK_IDLE_TIMEOUT_MS",
      "Meeting calendar initial cloud correction must have a bounded idle timeout.",
    ],
    [
      "const MEETING_INITIAL_CLOUD_RECHECK_DELAY_MS = 120",
      "Meeting calendar initial cloud correction must start shortly after local first paint.",
    ],
    [
      "const MEETING_INITIAL_CLOUD_RECHECK_IDLE_TIMEOUT_MS = 900",
      "Meeting calendar initial cloud correction idle timeout must stay bounded for cloud-first users.",
    ],
    [
      "void load({\n        includeCloud: false,\n        interruptCloud: false,\n        preserveVisibleMeetings: true,\n        includeUnindexedFallback: false,\n      });",
    ...
    assertSourceIncludes(files.meetingScheduleShell, meetingScheduleShell, snippet, message);
  }
  assertSourceIncludes(
    files.meetingScheduleShell,
    meetingScheduleShell,
  ...
  for (const [snippet, message] of [
    ...
    [
      ...
      "Meeting detail opens must reuse the local-first open path.",
    ],
    [
  ...
  for (const [snippet, message] of [
    ...
    assertSourceExcludes(files.syncShell, syncShell, snippet, message);
  }
  ...
  for (const [snippet, message] of [
    ...
    [
      'architecture_target: "cloud-master-local-hot-cache"',
      "Sync ack ledger replay proof must align with cloud-master/local-hot-cache.",
    ],
    [
    ...
    [
      ...
      "Sync ack ledger replay proof must reuse preflight fixture identity.",
    ],
    [
    ...
    [
      "can_mark_local_rows_synced_now: false",
      "Sync ack ledger replay proof must not allow marking local rows synced.",
    ],
    [
      "reads_page_body_text: false",
      "Sync ack ledger replay proof must not read page body text.",
    ],
    [
      "reads_database_row_values: false",
      "Sync ack ledger replay proof must not read database row values.",
    ],
    [
      "reads_comment_bodies: false",
      "Sync ack ledger replay proof must not read comment bodies.",
    ],
    [
      "reads_file_bytes: false",
      "Sync ack ledger replay proof must not read file bytes.",
    ],
    [
      "reads_secret_values: false",
      "Sync ack ledger replay proof must not read secrets.",
    ],
    [
      "sends_network_requests: false",
    ...
    [
      ...
      "Sync ack ledger replay proof must not write workspace data.",
    ],
    [
      "mutates_local_sync_log: false",
    ...
    [
      ...
      "Sync ack ledger replay proof must prove duplicate replay creates no batches.",
    ],
    [
      "proof-idempotent-batch",
      "Sync ack ledger replay proof must assert idempotent batch replay.",
    ],
    [
    ...
    [
      ...
      "Sync ack ledger replay proof must assert metadata-only proof boundaries.",
    ],
    [
      "proof-rollback-zeroes-fixture",
      "Sync ack ledger replay proof must assert rollback clears fixtures.",
    ],
    [
      "proof-cloud-runner-still-blocked",
      "Sync ack ledger replay proof must keep real cloud runner blocked.",
    ],
  ]) {
    assertSourceIncludes(
      files.syncAckLedgerReplayProof,
  ...
  for (const [snippet, message] of [
    ...
    [
      ...
      "Sync replay test API guard must identify the replay test route.",
    ],
    [
      'path: "/api/sync/replay-test"',
      "Sync replay test API guard must target the replay test route.",
    ],
    [
      "can_run_replay_now: false",
    ...
    [
      "can_write_server_data_now: false",
      "Sync replay test API guard must not write server data.",
    ],
    [
      "can_upload_workspace_data_now: false",
      "Sync replay test API guard must not upload workspace data.",
    ],
    [
      "can_acknowledge_rows_now: false",
      "Sync replay test API guard must not acknowledge rows.",
    ],
    [
      "can_mark_local_rows_synced_now: false",
      "Sync replay test API guard must not mark local rows synced.",
    ],
    [
    ...
    [
      ...
      "Sync replay test API guard must expose no request argument.",
    ],
    [
      "endpoint_disabled: true",
      "Sync replay test API guard must stay disabled.",
    ],
    [
      "accepts_disposable_replay_payload: false",
      "Sync replay test API guard must not accept replay payloads.",
    ],
    [
      "uses_production_workspace: false",
      "Sync replay test API guard must refuse production workspace usage.",
    ],
    [
      "reads_page_body_text: false",
      "Sync replay test API guard must not read page body text.",
    ],
    [
      "reads_database_row_values: false",
      "Sync replay test API guard must not read database row values.",
    ],
    [
      "reads_comment_bodies: false",
      "Sync replay test API guard must not read comment bodies.",
    ],
    [
      "reads_file_bytes: false",
      "Sync replay test API guard must not read file bytes.",
    ],
    [
      "reads_secret_values: false",
      "Sync replay test API guard must not read secrets.",
    ],
    [
      "reads_environment_values: false",
      "Sync replay test API guard must not read environment values.",
    ],
    [
    ...
    [
      "planned-disposable-metadata-only",
      "Sync replay test API guard must define metadata-only request schema.",
    ],
    [
      "planned-disabled-replay-receipt-only",
      "Sync replay test API guard must define disabled receipt response schema.",
    ],
    [
  ...
  for (const [snippet, message] of [
    ...
    [
      "request.json",
      "Sync replay test route must not read JSON bodies while disabled.",
    ],
    [
      "request.formData",
      "Sync replay test route must not read form bodies while disabled.",
    ],
  ]) {
    assertSourceExcludes(
      files.syncReplayTestRoute,
  ...
  for (const [snippet, message] of [
    ...
    [
      "导出 ack/retry 回放证明",
      "Sync UI must expose the ack/retry replay proof export.",
    ],
    [
      "纯内存合成证明",
      "Sync UI must explain the proof is pure in-memory synthetic proof.",
    ],
    [
      "不会修改本地 sync_log",
  ...
  for (const [snippet, message] of [
    [
      "page.content_text",
      "Sync ack ledger replay proof must not access page content text.",
    ],
    [
      "field_values",
      "Sync ack ledger replay proof must not access database row values.",
    ],
    [
      "comment.body",
      "Sync ack ledger replay proof must not access comment bodies.",
    ],
    [
      "file.dataUrl",
      "Sync ack ledger replay proof must not access file bytes.",
    ],
    [
      "file.textContent",
      "Sync ack ledger replay proof must not access file text.",
    ],
    [
      "fetch(",
      "Sync ack ledger replay proof must not call network APIs.",
    ],
    [
      "process.env",
      "Sync ack ledger replay proof must not read environment variables.",
    ],
    [
      "localStorage.setItem",
      "Sync ack ledger replay proof must not write browser storage.",
    ],
    [
      "db.run",
      "Sync ack ledger replay proof must not mutate the local database.",
    ],
    [
      "INSERT INTO",
      "Sync ack ledger replay proof must not insert rows.",
    ],
    [
      "UPDATE ",
      "Sync ack ledger replay proof must not update rows.",
    ],
    [
      "DELETE FROM",
      "Sync ack ledger replay proof must not delete rows.",
    ],
  ]) {
    assertSourceExcludes(
      files.syncAckLedgerReplayProof,
  ...
  for (const [snippet, message] of [
    [
      "page.content_text",
      "Sync replay test API guard must not access page content text.",
    ],
    [
      "field_values",
      "Sync replay test API guard must not access database row values.",
    ],
    [
      "comment.body",
      "Sync replay test API guard must not access comment bodies.",
    ],
    [
      "file.dataUrl",
      "Sync replay test API guard must not access file bytes.",
    ],
    [
      "file.textContent",
      "Sync replay test API guard must not access file text.",
    ],
    [
      "fetch(",
      "Sync replay test API guard must not call network APIs.",
    ],
    [
      "process.env",
      "Sync replay test API guard must not read environment variables.",
    ],
    [
      "localStorage.setItem",
      "Sync replay test API guard must not write browser storage.",
    ],
    [
      "db.run",
      "Sync replay test API guard must not mutate the local database.",
    ],
    [
      "INSERT INTO",
      "Sync replay test API guard must not insert rows.",
    ],
    [
      "UPDATE ",
      "Sync replay test API guard must not update rows.",
    ],
    [
      "DELETE FROM",
      "Sync replay test API guard must not delete rows.",
    ],
  ]) {
    assertSourceExcludes(
      files.syncReplayTestApiStub,
  ...
  for (const stub of apiStubRows) {
    ...
    } else if (stub.id === "sync-replay-test") {
      assertRouteGuard(
        routeFile,
        "buildSyncReplayTestApiDisabledResponse",
        routeLabel
  ...
  assertRouteExport(
    files.aiSuggestPositionTagsRoute,
    "POST",
    "POST /api/ai/suggest-position-tags"
  );
  for (const [snippet, message] of [
    [
    ...
    [
      ...
      "Anthropic provider timeouts must use a typed error so helper routes return stable retryable responses.",
    ],
    [
    ...
    [
      ...
      "Anthropic provider timeouts must surface as typed retryable errors.",
    ],
    [
      "clearTimeout(timeout)",
      "Anthropic provider timeout timers must be cleared after fetch settles.",
    ],
    [
  ...
  for (const [file, source] of [
    ...
    assertSourceExcludes(
      ...
      "AI helper routes must not fetch Anthropic directly; use fetchAnthropicMessagesWithTimeout."
    );
  }
  assertRouteExport(
  ...
  assertSourceIncludes(
    ...
    "Cloud manifest route verifier must emit a stable local receipt format."
  );
  for (const [snippet, message] of [
    [
      "ROUTE_PATH",
      "Cloud manifest route verifier must request the manifest compare route.",
    ],
    [
      "export async function GET()",
      "Cloud manifest route verifier must assert the route accepts no request argument.",
    ],
    [
    ...
    [
      ...
      "Cloud manifest route verifier must send a query marker.",
    ],
    [
    ...
    [
      "requiredTopLevelFalseFlags",
      "Cloud manifest route verifier must assert disabled top-level flags.",
    ],
    [
      "requiredBoundaryFalseFlags",
      "Cloud manifest route verifier must assert disabled boundary flags.",
    ],
    [
      "requiredForbiddenRequestFields",
      "Cloud manifest route verifier must assert forbidden request fields.",
    ],
    [
      "requiredForbiddenResponseFields",
      "Cloud manifest route verifier must assert forbidden response fields.",
    ],
    [
    ...
    [
      "sends_external_network_requests: false",
      "Cloud manifest route verifier must not send external network requests.",
    ],
    [
      "reads_browser_storage: false",
      "Cloud manifest route verifier must not read browser storage.",
    ],
    [
      "connects_cloud_services: false",
    ...
    [
      "reads_page_body_text: false",
      "Cloud manifest route verifier must not read page bodies.",
    ],
    [
      "reads_database_row_values: false",
      "Cloud manifest route verifier must not read database values.",
    ],
    [
      "reads_file_bytes: false",
      "Cloud manifest route verifier must not read file bytes.",
    ],
    [
      "cloud_sync_can_start_now: false",
  ...
  for (const [snippet, message] of [
    [
      ...
      "Cloud ACK/cache safety report must have a stable format.",
    ],
    [
    ...
    [
      'architecture_target: "cloud-master-user-selected-local-copy"',
      "Cloud ACK/cache safety report must align with cloud master plus user-selected local copies.",
    ],
    [
      "can_show_local_saved_now: true",
      "Cloud ACK/cache safety report must always allow immediate local saved UI.",
    ],
    [
      "can_show_cloud_confirmed_now",
      "Cloud ACK/cache safety report must explicitly gate the cloud-confirmed UI state.",
    ],
    [
      "can_switch_device_now",
      "Cloud ACK/cache safety report must explicitly gate device handoff.",
    ],
    [
      "can_queue_cache_rebuild_confirmation_now",
      "Cloud ACK/cache safety report must separate cache rebuild confirmation from direct cache clearing.",
    ],
    [
      "can_clear_local_cache_now: false",
      "Cloud ACK/cache safety report must never clear local cache from the sync page.",
    ],
    [
      "can_mark_local_rows_synced_now: false",
      "Cloud ACK/cache safety report must not mark local rows as synced.",
    ],
    [
    ...
    [
      "reads_file_bytes: false",
      "Cloud ACK/cache safety report must not read file bytes.",
    ],
    [
      "sends_network_requests: false",
    ...
    [
      "clears_local_cache: false",
      "Cloud ACK/cache safety report must not clear local cache.",
    ],
    [
      "marks_local_rows_synced: false",
    ...
    [
      "local-cache-clear-guard",
      "Cloud ACK/cache safety report must include a direct local-cache clear guard.",
    ],
    [
      "buildCloudAckCacheSafetyReport",
      "Cloud ACK/cache safety report builder must be exported.",
    ],
  ]) {
    assertSourceIncludes(
      files.cloudAckCacheSafetyReport,
  ...
  for (const [snippet, message] of [
    ...
    [
      ...
      "Sync UI must provide a stable cloud ACK/cache safety panel anchor.",
    ],
    [
      "云端确认与本地缓存安全",
    ...
    [
      "同步页不能直接显示“云端已确认”或清理本地缓存",
      "Sync UI must explain why durable ACK is required before cloud-confirmed/cache-clear states.",
    ],
  ]) {
    assertSourceIncludes(files.syncShell, syncShell, snippet, message);
  }
  ...
  for (const [sourceLabel, source, snippet, message] of [
    ...
    [
      ...
      "Database detail pages must refresh a bounded local preview after background cloud hydration completes.",
    ],
    [
      files.databaseShell,
      databaseShell,
    ...
    [
      ...
      "Database detail pages must hydrate remaining rows in bounded idle batches after first paint.",
    ],
    [
      files.databaseShell,
      databaseShell,
    ...
    [
      ...
      "Database detail pages must hydrate local rows through an idle batch loop.",
    ],
    [
      files.databaseShell,
      databaseShell,
  ...
  for (const [sourceLabel, source, snippet, message] of [
    ...
    [
      ...
      "Sidebar must expose a testable account cloud-sync coordinator control.",
    ],
    [
      files.sidebar,
      sidebar,
    ...
    [
      ...
      "Sidebar cloud-sync control must open the Sync module for pending, settings, knowledge replay, failed, or manual-review queues.",
    ],
    [
      files.sidebar,
      sidebar,
    ...
    [
      ...
      sidebar,
      "/modules/sync#page-pending-upload-queue",
      "Sidebar cloud-sync control must deep-link page pending rows to the page upload queue.",
    ],
    [
      files.sidebar,
      sidebar,
    ...
    [
      ...
      "Sync module must expose a stable database pending upload queue anchor.",
    ],
    [
      files.syncShell,
      syncShell,
      'data-testid="database-pending-upload-queue"',
      "Sync module must expose a testable database pending upload queue anchor.",
    ],
    [
      files.sidebar,
      sidebar,
    ...
    [
      ...
      "Sidebar cloud-sync control must expose total pending inputs.",
    ],
    [
      files.sidebar,
      sidebar,
      "data-failed-total={accountSync.failedTotal}",
      "Sidebar cloud-sync control must expose failed sync count.",
    ],
    [
      files.sidebar,
      sidebar,
      "data-manual-review-total={accountSync.manualReviewTotal}",
      "Sidebar cloud-sync control must expose manual-review sync count.",
    ],
    [
      files.sidebar,
      sidebar,
      "data-knowledge-pending-total={accountSync.knowledgePendingTotal}",
      "Sidebar cloud-sync control must include knowledge-domain pending inputs.",
    ],
    [
      files.sidebar,
      sidebar,
      "页面同步未开启",
      "Sidebar cloud-sync tooltip must distinguish disabled page sync from sync errors.",
    ],
    [
      files.sidebar,
      sidebar,
    ...
    [
      ...
      "Page and sidebar lists must render a bounded user-selected hot-cache metadata snapshot before full local scans.",
    ],
    [
      files.usePages,
      usePages,
      "listHotCachePageMetadata",
      "Page and sidebar lists must use a dedicated bounded metadata query for hot-cache first paint.",
    ],
    [
      files.usePages,
      usePages,
      "metadataRecentLimitForHotCachePreferences(preferences)",
      "Page and sidebar hot-cache first paint must follow the user's recent-range preference.",
    ],
    [
      files.usePages,
      usePages,
      "scheduleDeferredMetadataHydration(setPages)",
      "Page and sidebar lists must defer full local metadata hydration until after fast hot-cache paint.",
    ],
    [
      files.usePages,
      usePages,
    ...
    [
      ...
      "Local queries must expose a bounded page hot-cache metadata reader.",
    ],
    [
      files.localQueries,
      localQueries,
      "WITH RECURSIVE",
      "Page hot-cache metadata must include parent chains without reading page bodies.",
    ],
    [
      files.localQueries,
      localQueries,
      "recent_seed(id)",
      "Page hot-cache metadata must include a bounded recent-page seed.",
    ],
    [
      files.localQueries,
      localQueries,
      "root_seed(id)",
      "Page hot-cache metadata must include a bounded top-level page seed for sidebar navigation.",
    ],
    [
      files.usePages,
      usePages,
    ...
    [
      ...
      "Local page content hydration must expose a single-page projection for route opens.",
    ],
    [
      files.localQueries,
      localQueries,
    ...
    [
      ...
      "Sidebar page tree must show a local-cache-aware empty/loading state instead of implying data is missing.",
    ],
    [
      files.pageTree,
      pageTree,
      'data-testid="sidebar-page-tree-windowing-status"',
      "Sidebar page tree must expose root page windowing status when large imports are folded.",
    ],
    [
      files.pageTree,
      pageTree,
      'data-testid="sidebar-page-tree-child-windowing-status"',
      "Sidebar page tree must expose child page windowing status when large parents are folded.",
    ],
    [
      files.pageTree,
      pageTree,
      'data-local-cache-ready={String(dbReady)}',
      "Sidebar page tree empty state must distinguish local cache preparation from a truly empty workspace.",
    ],
    [
      files.pageTree,
      pageTree,
      'data-local-first-windowing="true"',
      "Sidebar page tree windowing status must identify the local-first performance guard.",
    ],
    [
      files.pageTree,
      pageTree,
      "data-visible-root-pages={visibleRootPages.length}",
      "Sidebar page tree must expose the visible root page count.",
    ],
    [
      files.pageTree,
      pageTree,
      "data-visible-child-pages={visibleChildren.length}",
      "Sidebar page tree must expose the visible child page count.",
    ],
    [
      files.pageTree,
      pageTree,
      "正在准备本地缓存，页面会先从热缓存显示。",
      "Sidebar page tree must tell the owner when the local cache is still preparing.",
    ],
    [
      files.pageTree,
      pageTree,
      "先显示 {visibleRootPages.length}/{rootPages.length} 个页面",
      "Sidebar page tree must explain that large root page lists are partially rendered for fluidity.",
    ],
    [
      files.pageTree,
      pageTree,
      "先显示 {visibleChildren.length}/{children.length} 个子页面",
      "Sidebar page tree must explain that large child page lists are partially rendered for fluidity.",
    ],
    [
      files.pageTree,
      pageTree,
  ...
  for (const [snippet, message] of [
    ...
    const assertFn = snippet.startsWith("page.content")
      ? assertSourceExcludes
      : assertSourceIncludes;
    assertFn(files.pageMetadataSearch, pageMetadataSearch, snippet, message);
  }
  assertSourceIncludes(
    files.projectsShell,
    projectsShell,
