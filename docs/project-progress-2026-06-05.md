...
- Files 模块的本地文件列表新增格式/能力筛选：全部、HTML、Markdown、原生预览、可编辑导入、
  表格入库、元数据复核、本地留存。筛选只读取本地文件 metadata 和格式路线，不读取文件正文或 bytes。
...
- Notes 模块新增本地同步块 registry 面板，扫描本地页面 HTML 中的 `data-sync-id` 和页面标题，
  展示同步组、实例数、跨页面复用数和下一步复核提示。
- Registry 可以导出 metadata-only JSON；导出不包含同步块正文、页面正文、数据库 row values、
  文件 bytes、云端数据或 AI 内容。
- 当前阶段不做跨页面自动同步、不定义权限继承、不处理冲突；这些仍留到云同步和协作阶段。
